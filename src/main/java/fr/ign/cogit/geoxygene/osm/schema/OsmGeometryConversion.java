...
public class OsmGeometryConversion {
    ...
    /**
     * Converts a way into a {@link ILineString} using a collection of the
     * nodes, as ways only describe the node id of the vertices. The coordinates
     * are projected in the given coordinate system.
     * 
     * @param way
     * @param nodes
     * @return
    ...
    public ILineString convertOSMLine(OSMWay way, Collection<OSMResource> nodes,
            boolean crsConversion) throws Exception {
        ...
        for (long index : way.getVertices()) {
            ...
            if (vertex != null && crsConversion) {
                IDirectPosition pt = CRSConversion
                        .changeCRS(new GM_Point(vertex.getPosition()), "4326",
                                epsg, false, true)
                        .coord().get(0);
                coord.add(pt);
            } else if (vertex != null)
                coord.add(vertex.getPosition());
        }
     ...
     * Converts a way into a {@link ILineString} using a collection of the
     * nodes, as ways only describe the node id of the vertices. The coordinates
     * are projected in Lambert93 so it doesn't work on non-French data.
     * 
     * @param way
     * @param nodes
     * @return
    ...
    /**
     * Converts a way into a {@link IPolygon} using a collection of the nodes,
     * as ways only describe the node id of the vertices. If the way does not
    ...
    public IPoint convertOsmPoint(OSMNode node, boolean crsConversion)
            throws Exception {
        IDirectPosition pt = null;
        if (crsConversion)
            pt = CRSConversion.changeCRS(new GM_Point(node.getPosition()),
                    "4326", epsg, false, true).coord().get(0);
        else
            pt = node.getPosition();
        return pt.toGM_Point();
    }
